    ...
    <DropdownMenu>
      <DropdownMenuTrigger as-child>
        <Button variant="ghost" size="sm" class="-ml-3 h-8 data-[state=open]:bg-accent">
          <span>{{ title }}</span>
          <ILucideArrowDown v-if="column.getIsSorted() === 'desc'" class="ml-2 h-4 w-4" />
          <ILucideArrowUp v-else-if="column.getIsSorted() === 'asc'" class="ml-2 h-4 w-4" />
          <ILucideArrowUpDown v-else-if="sortable" class="ml-2 h-4 w-4" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="start">
        ...
        <template v-if="sortable">
          <DropdownMenuItem @click="column.toggleSorting(false)">
            <ILucideArrowUp class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Asc
          </DropdownMenuItem>
          <DropdownMenuItem @click="column.toggleSorting(true)">
            <ILucideArrowDown class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Desc
          </DropdownMenuItem>
        ...
        <template v-if="groupable">
          <DropdownMenuItem v-if="!column.getIsGrouped()" @click="column.toggleGrouping()">
            <ILucideGroup class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Group by {{ title }}
          </DropdownMenuItem>
          <DropdownMenuItem v-else @click="column.toggleGrouping()">
            <ILucideUngroup class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Ungroup
          </DropdownMenuItem>
        ...
        <template v-if="pinnable">
          <DropdownMenuItem :disabled="column.getIsPinned() === 'left'" @click="column.pin('left')">
            <ILucidePin class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Pin to left
          </DropdownMenuItem>
          <DropdownMenuItem
          ...
          >
            <ILucidePinOff class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Pin to right
          </DropdownMenuItem>
        ...
        <template v-if="filterable">
          <DropdownMenuItem @click="column.setFilterValue('')">
            <ILucideFilterX class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
            Clear filter
          </DropdownMenuItem>
        ...
        <DropdownMenuItem @click="column.toggleVisibility(false)">
          <ILucideEyeOff class="mr-2 h-3.5 w-3.5 text-muted-foreground/70" />
          Hide column
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
